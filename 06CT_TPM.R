...
ko <- read.table("raw data - acf:jgi files/041406CT_assembled_derep.ko", sep="\t")
phylo <- read.table("raw data - acf:jgi files/041406CT_assembled.phylodist", sep="\t", quote = "")
head(ko)
head(phylo)
...
cov <- read.table("raw data - acf:jgi files/06CT_covstats.txt")
cov$rg <- cov$V7+cov$V8
cov <- rename(cov, gene=V1, flg=V3)
cov <- subset(cov, select=c(gene, flg, rg))
head(cov)
rl <- read.table("raw data - acf:jgi files/06CT_rl_sort.txt")
...
MT_06CT <- merge(genes, reads, all=TRUE)
MT_06CT[is.na(MT_06CT)] <- 0
###calculate T###
G <- ((sum(MT_06CT$rg)*sum(MT_06CT$rl))/sum(MT_06CT$flg))
G
###create a new variable with TPM for each row (scaffold/gene_id)####
TPM <- transform(MT_06CT, TPM=(rg*rl*1e6)/(flg*G))
TPM$KO_num <- as.character(TPM$KO_num)
TPM$KO_num[is.na(TPM$KO_num)] <- "CDS without KO annotation"

write.csv(TPM, "06CT_TPM_allCDS.csv", row.names = FALSE)
