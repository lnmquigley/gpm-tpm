...
ko <- read.table("raw data - acf:jgi files/041413BS_assembled_derep.ko", sep="\t")
phylo <- read.table("raw data - acf:jgi files/13BS_assembled.phylodist", sep="\t", quote = "")
head(ko)
head(phylo)
...
cov <- read.table("raw data - acf:jgi files/13BS_covstats.txt")
cov$rg <- cov$V7+cov$V8
cov <- rename(cov, gene=V1, flg=V3)
cov <- subset(cov, select=c(gene, flg, rg))
head(cov)
rl <- read.table("raw data - acf:jgi files/13BS_rl_sort.txt")
...
MT_13BS <- merge(genes, reads, all = TRUE)
MT_13BS[is.na(MT_13BS)] <- 0
###calculate T###
G <- ((sum(MT_13BS$rg)*sum(MT_13BS$rl))/sum(MT_13BS$flg))
G
###create a new variable with TPM for each row (scaffold/gene_id)####
TPM <- transform(MT_13BS, TPM=(rg*rl*1e6)/(flg*G))
TPM$KO_num <- as.character(TPM$KO_num)
TPM$KO_num[is.na(TPM$KO_num)] <- "CDS without KO annotation"

write.csv(TPM, "13BS_TPM_allCDS.csv", row.names = FALSE)
