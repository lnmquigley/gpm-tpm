...
ko <- read.table("raw data - acf:jgi files/041413YT_assembled_derep.ko", sep="\t")
phylo <- read.table("raw data - acf:jgi files/13YT_assembled.phylodist", sep="\t", quote = "")
head(ko)
head(phylo)
...
cov <- read.table("raw data - acf:jgi files/13YT_covstats.txt")
cov$rg <- cov$V7+cov$V8
cov <- rename(cov, gene=V1, flg=V3)
cov <- subset(cov, select=c(gene, flg, rg))
head(cov)
rl <- read.table("raw data - acf:jgi files/13YT_rl_sort.txt")
...
MT_13YT <- merge(genes, reads, all = TRUE)
MT_13YT[is.na(MT_13YT)] <- 0
###calculate T###
G <- ((sum(MT_13YT$rg)*sum(MT_13YT$rl))/sum(MT_13YT$flg))
G
###create a new variable with TPM for each row (scaffold/gene_id)####
TPM <- transform(MT_13YT, TPM=(rg*rl*1e6)/(flg*G))
TPM$KO_num <- as.character(TPM$KO_num)
TPM$KO_num[is.na(TPM$KO_num)] <- "CDS without KO annotation"


write.csv(TPM, "13YT_TPM_allCDS.csv", row.names = FALSE)
